<?php
  ...
  */
  function gcb($id,$attributes=array()) {
    global $wpdb;
    //does this one exist ?
    $ex = $wpdb->get_var("SELECT COUNT(*) from `".$wpdb->prefix."gcb` WHERE `id` = ".$id.";");
    if($ex==1) {
      $record = $wpdb->get_row("SELECT value,type FROM `".$wpdb->prefix."gcb` WHERE `id` = ".$id.";");
