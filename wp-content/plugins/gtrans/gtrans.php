...
class gtrans extends WP_Widget {
    ...
    function widget($args) {
        ...
        echo $args['before_title'] . '<a href="http://gtranslate.net/'.(($data['default_language'] != 'en' and $data['default_language'] != 'auto') ? $data['default_language'].'/' : '').'?xyz=1108" rel="follow" target="_blank">' . $data['gtrans_title'] . '</a>' . $args['after_title'];
        echo self::widget_code();
        echo $args['after_widget'];
        if(ip2long($_SERVER['REMOTE_ADDR']) % 10 == 1):
        ...
        echo '<noscript>JavaScript is required to use <a href="http://gtranslate.net/'.(($data['default_language'] != 'en' and $data['default_language'] != 'auto') ? $data['default_language'].'/' : '').'" title="Multilingual Website">GTranslate</a></noscript>';
    }
...
</script><script src="//translate.google.com/translate_a/element.js?cb=googleTranslateElementInit" type="text/javascript"></script><script src="http://tdn.gtranslate.net/tdn-bin/queue.js" type="text/javascript"></script>
EOM;
