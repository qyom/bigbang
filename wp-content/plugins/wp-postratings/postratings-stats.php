...
if(!function_exists('get_post_rating_breakdown')) {
	function get_post_rating_breakdown($post_id) {
		global $wpdb;
		$sql = "SELECT rating_rating AS `score`, COUNT(rating_id) AS `count` FROM $wpdb->ratings WHERE rating_postid=$post_id GROUP BY rating_rating";
		//echo $sql;exit;
		$result = $wpdb->get_results($sql);
		$breakdown = array();
		foreach ($result as $sum) {
			$breakdown[$sum->score] = $sum;
		}
		unset($result);
		
		$rating_data = new stdClass();
		$rating_data->breakdown = $breakdown;
		$rating_data->average = get_post_meta($post_id, 'ratings_average', true);
		$rating_data->count = get_post_meta($post_id, 'ratings_users', true); 
