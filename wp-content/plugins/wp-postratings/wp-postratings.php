...
// Do it the V way
function the_ratings_V($post_id) {
	// Check To See Whether User Has Voted
	//$user_voted = check_rated($ratings_id);
	//$is_allowed = check_allowtorate();
	
	$rating_data = get_post_rating_breakdown($post_id);
	
	/*echo "<pre>";
	var_dump($rating_data);
	echo "</pre>";  
	exit;*/
	wp_register_script( "rating_script", get_template_directory_uri().'/js/rating_script.js' );
